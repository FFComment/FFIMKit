...
@interface EAMMesageChatVC ()<ICChatBoxViewControllerDelegate,UITableViewDelegate,UITableViewDataSource,ICRecordManagerDelegate,UIViewControllerTransitioningDelegate,UIViewControllerAnimatedTransitioning,BaseCellDelegate>
{
    CGRect _smallRect;
    CGRect _bigRect;
...
// send text message
- (void) chatBoxViewController:(ICChatBoxViewController *)chatboxViewController sendTextMessage:(NSString *)messageStr
{
    if (messageStr && messageStr.length > 0) {
        [self sendTextMessageWithContent:messageStr];
        [self otherSendTextMessageWithContent:messageStr];
        if ([self.delegate respondsToSelector:@selector(chatVCSendMessage:)]) {
            [self.delegate chatVCSendMessage:messageStr];
        }
    }
}
...
// send image message
- (void) chatBoxViewController:(ICChatBoxViewController *)chatboxViewController sendImageMessage:(UIImage *)image imagePath:(NSString *)imgPath
{
    if (image && imgPath) {
        [self sendImageMessageWithImgPath:imgPath];
        if ([self.delegate respondsToSelector:@selector(chatVCSendMessage:)]) {
            [self.delegate chatVCSendMessage:imgPath];
        }
    }
}
...
- (void) chatBoxViewController:(ICChatBoxViewController *)chatboxViewController sendVoiceMessage:(NSString *)voicePath
{
    ...
    if (voicePath) {
        ...
        [self messageSendSucced:messageF];
        if ([self.delegate respondsToSelector:@selector(chatVCSendMessage:)]) {
            [self.delegate chatVCSendMessage:voicePath];
        }
    }
}
...
- (void)voiceWillDragout:(BOOL)inside
{
    if (inside) {
        ...
        _voiceHud.image  = [UIImage imageWithContentsOfFile:voice_1];
    } else {
        [_timer setFireDate:[NSDate distantFuture]];
        self.voiceHud.animationImages  = nil;
        NSString *cancelVoice = [FF_BoundlePath ff_imagePathWithName:@"cancelVoice" bundle:@"FFIMKit" targetClass:[self class] oftype:@"png"];
        self.voiceHud.image = [UIImage imageWithContentsOfFile:cancelVoice];
    }
}
- (void)progressChange
{
...
- (void)voiceRecordSoShort
{
    [self timerInvalue];
    self.voiceHud.animationImages = nil;
    NSString *voiceShort = [FF_BoundlePath ff_imagePathWithName:@"voiceShort" bundle:@"FFIMKit" targetClass:[self class] oftype:@"png"];
    self.voiceHud.image = [UIImage imageWithContentsOfFile:voiceShort];
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        self.voiceHud.hidden = YES;
    });
}
...
// play voice
- (void)chatVoiceTaped:(ICMessageFrame *)messageFrame voiceIcon:(UIImageView *)voiceIcon redView:(UIView *)redView
{
