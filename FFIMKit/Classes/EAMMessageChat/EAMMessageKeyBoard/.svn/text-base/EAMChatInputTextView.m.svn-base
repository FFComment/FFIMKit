...
#import "EAMEmojiConvertToString.h"//将表情图片转换成code：如[em_1]

@implementation EAMChatInputTextView
...
-(NSString *)plainText{
    ...
    [result enumerateAttribute:NSAttachmentAttributeName inRange:range options:0 usingBlock:^(id value, NSRange range, BOOL *stop) {
        if (value && [value isKindOfClass:[EAMEmojiTextAttachment class]]) {
            EAMEmojiTextAttachment *attach = (EAMEmojiTextAttachment *)value;
            [result deleteCharactersInRange:range];
            [result insertAttributedString:[[NSAttributedString alloc] initWithString:[EAMEmojiConvertToString convertToCommonEmoticons:[NSString stringWithFormat:@",%@",attach.emoName]]] atIndex:range.location];
        }
    }];
