...
- (ICChatBoxMoreView *)chatBoxMoreView
{
    if (nil == _chatBoxMoreView) {
        ...
        // 创建Item
        NSString *more_face = [FF_BoundlePath ff_imagePathWithName:@"more_face" bundle:@"FFIMKit" targetClass:[self class] oftype:@"png"];
        NSString *more_pic = [FF_BoundlePath ff_imagePathWithName:@"more_pic" bundle:@"FFIMKit" targetClass:[self class] oftype:@"png"];
        NSString *more_camera = [FF_BoundlePath ff_imagePathWithName:@"more_camera" bundle:@"FFIMKit" targetClass:[self class] oftype:@"png"];
//        NSString *sharemore_sight = [FF_BoundlePath ff_imagePathWithName:@"sharemore_sight" bundle:@"FFIMKit" targetClass:[self class] oftype:@"png"];
//        NSString *sharemore_wallet = [FF_BoundlePath ff_imagePathWithName:@"sharemore_wallet" bundle:@"FFIMKit" targetClass:[self class] oftype:@"png"];
        ICChatBoxMoreViewItem *faceItem = [ICChatBoxMoreViewItem createChatBoxMoreItemWithTitle:@"表情" imageName:more_face];
        ICChatBoxMoreViewItem *photosItem = [ICChatBoxMoreViewItem createChatBoxMoreItemWithTitle:@"相册" imageName:more_pic];
        ICChatBoxMoreViewItem *takePictureItem = [ICChatBoxMoreViewItem createChatBoxMoreItemWithTitle:@"拍照" imageName:more_camera];
//        ICChatBoxMoreViewItem *videoItem   = [ICChatBoxMoreViewItem createChatBoxMoreItemWithTitle:@"小视频" imageName:sharemore_sight];
//        ICChatBoxMoreViewItem *docItem   = [ICChatBoxMoreViewItem createChatBoxMoreItemWithTitle:@"文件" imageName:sharemore_wallet];
        [_chatBoxMoreView setItems:[[NSMutableArray alloc] initWithObjects:faceItem, photosItem, takePictureItem, nil]];
    }
...
{
    if (itemType == ICChatBoxItemFace) { // 表情
        [[NSNotificationCenter defaultCenter] postNotificationName:@"GXEmotionShowChatBoxFaceView" object:nil];
    } else if (itemType == ICChatBoxItemAlbum){    // 相册
    ...
    } else if (itemType == ICChatBoxItemCamera) {  // 拍照
    ...
    } else if (itemType == ICChatBoxItemDoc) {  // 文件
...
- (void)chatBox:(ICChatBox *)chatBox changeStatusForm:(ICChatBoxStatus)fromStatus to:(ICChatBoxStatus)toStatus
{
    if (toStatus == ICChatBoxStatusShowKeyboard) {  // 显示键盘
        ...
        WEAKSELF
        [UIView animateWithDuration:0.3 animations:^{
            if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(chatBoxViewController:didChangeChatBoxHeight:)]) {
                [weakSelf.delegate chatBoxViewController:self didChangeChatBoxHeight:HEIGHT_TABBAR];
            }
        } completion:^(BOOL finished) {
            [self.chatBoxFaceView removeFromSuperview];
            [self.chatBoxMoreView removeFromSuperview];
        ...
        if (fromStatus == ICChatBoxStatusShowVoice || fromStatus == ICChatBoxStatusNothing) {
            ...
            [UIView animateWithDuration:0.3 animations:^{
                if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(chatBoxViewController:didChangeChatBoxHeight:)]) {
                    [weakSelf.delegate chatBoxViewController:self didChangeChatBoxHeight:HEIGHT_TABBAR + HEIGHT_CHATBOXVIEW];
                }
            }];
        } else {  // 表情高度变化
...
//    }
    ...
    [[ICRecordManager shareManager] startRecordingWithFileName:self.recordName completion:^(NSError *error) {
        if (error) {   // 加了录音权限的判断
            ...
            if ([weakSelf.delegate respondsToSelector:@selector(voiceDidStartRecording)]) {
                [weakSelf.delegate voiceDidStartRecording];
            }
        }
    }];
}
