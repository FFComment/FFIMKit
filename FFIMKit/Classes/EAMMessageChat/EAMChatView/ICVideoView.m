...
//    self.recordBtn.frame = CGRectMake(0, self.height-20-70, 70, 70);
    ...
    [label mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(weakSelf.videoLayerView.mas_centerX);
        make.centerY.equalTo(weakSelf.videoLayerView.mas_centerY);
    }];
...
- (void)recordVideoStarted
{
    ...
    [UIView animateWithDuration:kDurationTime animations:^{
        weakSelf.timeLine.frame = CGRectMake(weakSelf.timeLine.centerX, weakSelf.timeLine.top, 0, 1);
...
- (void)recordVideoCanceled
{
    ...
    [[ICVideoManager shareManager] stopRecordingVideo:^(NSString *path) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            [[ICVideoManager shareManager] cancelRecordingVideoWithFileName:weakSelf.videoName];
        });
    }];
}
