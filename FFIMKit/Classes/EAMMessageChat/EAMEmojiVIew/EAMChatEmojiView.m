...
@interface EAMChatEmojiView()
<UIScrollViewDelegate>
{
    EAMEmojiScrollView * scroll;
    EAMIconButton * selectIcon;
}
...
@implementation EAMChatEmojiView
...
- (void)addScroll:(CGRect)frame{
    scroll = [[EAMEmojiScrollView alloc]initWithFrame:frame];
...
- (void)addPageControl:(CGRect)frame{
    ...
    NSString *selecPath = [FF_BoundlePath ff_imagePathWithName:@"compose_keyboard_dot_selected" bundle:@"FFIMKit" targetClass:[self class] oftype:@"png"];
    [self.pageControl setCurrentPageIndicatorImage:[UIImage imageWithContentsOfFile:selecPath]];
    NSString *normalPath = [FF_BoundlePath ff_imagePathWithName:@"compose_keyboard_dot_normal" bundle:@"FFIMKit" targetClass:[self class] oftype:@"png"];
    [self.pageControl setPageIndicatorImage:[UIImage imageWithContentsOfFile:normalPath]];
...
- (void)addBottom:(CGRect)frame{
    ...
    //common
    EAMIconButton * common = [[EAMIconButton alloc]initWithFrame:CGRectMake(0, 0, ChatEmojiView_Bottom_W, ChatEmojiView_Bottom_H)];
    NSString *path = [FF_BoundlePath ff_imagePathWithName:@"ic_emoji_blue" bundle:@"FFIMKit" targetClass:[self class] oftype:@"png"];
    [common setImage:[UIImage imageWithContentsOfFile:path] forState:UIControlStateNormal];
...
- (void)common:(EAMIconButton*)sender{
    [self sender:sender];
    [self addEmjioWith:[EAMCommentEmoji class]];
}
...
- (void)sender:(EAMIconButton*)sender{
    if (sender) {
...
- (void)addEmjioWith:(Class)class{
    ...
    for (int i = 0; i < page_support; i++) {
        ...
        for (int j = 0; j < array.count - 1; j++) {
            ...
            EAMEmojiButton * button = [[EAMEmojiButton alloc]initWithFrame:frame];
        ...
        EAMEmojiButton * del_B = [[EAMEmojiButton alloc]initWithFrame:CGRectMake(EmojiView_Border+(count_lins-1)*(space+EmojiIMG_Width_Hight)+(i*kScreenWidth), EmojiView_Border+(EmojiIMG_Lines-1)*(EmojiIMG_Space_UP+EmojiIMG_Width_Hight), EmojiIMG_Width_Hight, EmojiIMG_Width_Hight)];
...
- (void)scrollInit{
    for (UIView * v in scroll.subviews) {
        if ([v isKindOfClass:[EAMEmojiButton class]]) {
...
-(void)selectorThisIcon:(EAMEmojiButton*)sender{
